...
PRESTATUS=$(sudo -s gcloud compute instances describe $INSTANCE --project=$PROJECTID --zone=$ZONE | grep  "status")
STATUS=${PRESTATUS:7}
if [ $STATUS = "RUNNING" ]; then
        echo "$(date "+%d.%m.%Y %T") $INSTANCE Instance is Running at"$(sudo -s gcloud compute instances describe $INSTANCE --project=$PROJECTID --zone=$ZONE| grep natIP | cut -d':'  -f2) |tee -a $LOGFILE
        exit
fi
...
if [ $STATUS = "TERMINATED" ]; then
        ...
        sudo -s gcloud compute instances start --quiet $INSTANCE --project=$PROJECTID --zone=$ZONE
